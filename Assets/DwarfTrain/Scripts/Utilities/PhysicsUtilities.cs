...
namespace Assets.DwarfTrain.Scripts.Utilities
{
    public class PhysicsUtilities
    {
        ...
        public static void FindArrowVelocityLateral(Vector3 source,
            ...
            float lateralSpeed,
            float maxHeight,
            Action<float, Vector3, float> arrowVelocityFoundAction,
            Action<float> cannotFindVelocityAction)
        {
            ...
            if (!foundArc)
            {
                cannotFindVelocityAction?.Invoke(lateralSpeed);
                return;
            }

            arrowVelocityFoundAction?.Invoke(lateralSpeed, force, gravity);
        }
